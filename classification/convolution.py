# Code from https://github.com/AgamChopra/conv4d
# and from https://github.com/pvjosue/pytorch_convNd
...
class convNd(nn.Module):
    ...
    def __init__(self,
        ...
        if rank==0 and self.kernel_initializer is None:
            ...
            self.kernel_initializer = lambda x: nn.init.uniform_(x, -k, k)
            ...
            if self.rank==0:
                ...
                if self.padding_mode == 'zeros':
                    input = F.pad(input.view(
                        ...
                        input.shape[2],
                        -1
                    ),padSize,'constant',0).view(inputShape)
                else:
                    input = F.pad(input.view(
                        input.shape[0],
                        input.shape[1],
                        input.shape[2],
                        -1
                    ),padSize,self.padding_mode).view(inputShape)
...
def Conv4d(
    ...
    return convNd(
        ...
        groups=groups, 
        dilation = dilation,
        kernel_initializer=lambda x: nn.init.constant_(x, w),  
        bias_initializer=lambda x: nn.init.constant_(x, b)
    )


def ConvTranspose4d(in_channels: int, 
                    out_channels: int,
                    kernel_size:int=2,
                    stride:int=1, 
                    padding:int = 0, 
